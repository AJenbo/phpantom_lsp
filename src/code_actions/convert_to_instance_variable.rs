...
fn collect_info(content: &str, cursor_offset: u32) -> Option<ConvertInfo> {
    with_parsed_program(content, "convert_to_instance_variable", |program, _content| {
        ...
        let assignment_info = find_assignment_in_block(block.statements.as_slice(), cursor_offset)?;
    ...
    match stmt {
        ...
        Statement::Try(t) => {
            ...
            for catch in t.catch_clauses.iter() {
                if let Some(r) =
                    find_assignment_in_block(catch.block.statements.as_slice(), cursor)
                {
                    return Some(r);
                }
            }
    ...
    match &if_stmt.body {
        IfBody::Statement(body) => {
            ...
            if let Some(ref else_clause) = body.else_clause {
                if let Some(r) = find_assignment_in_stmt(else_clause.statement, cursor) {
                    return Some(r);
                }
            }
        }
...
mod tests {
    ...
    fn replaces_all_occurrences_in_method() {
        ...
        assert_eq!(count, 3, "should replace all 3 occurrences, got: {}", result);
