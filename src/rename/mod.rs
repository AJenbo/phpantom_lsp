...
impl Backend {
    ...
    fn build_namespace_rename_edit(
        ...
        for file_uri in &all_uris {
            if vendor_prefixes.iter().any(|p| file_uri.starts_with(p.as_str())) {
                continue;
            }
            ...
            self.collect_namespace_decl_edits(
                &content,
                &old_prefix,
                &new_prefix,
                &mut file_edits,
            ...
            self.collect_use_statement_edits(
                &content,
                &old_prefix,
                &new_prefix,
                &mut file_edits,
        ...
        if let Some(ops) = self.build_namespace_psr4_rename_ops(&old_prefix, &new_prefix) {
            if !ops.is_empty() && self.supports_file_rename.load(Ordering::Acquire) {
                ...
                for (uri, edits) in changes {
                    let target_uri = ops
                        ...
                        .find_map(|(old_u, new_u)| {
                            ...
                            if uri_str.starts_with(old_str) {
                                let rest = &uri_str[old_str.len()..];
    ...
    fn collect_fqn_reference_edits(
        ...
        for span in &symbol_map.spans {
            let name = match &span.kind {
                SymbolKind::ClassReference { name, is_fqn: true, .. } => name,
                _ => continue,
            };
