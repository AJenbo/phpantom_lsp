...
| B4  | [`analyze` reports Blade type diagnostics six lines too early](todo/bugs.md#b4-analyze-reports-blade-type-diagnostics-six-lines-too-early)                                    | Medium      | Low         |
| B5  | ["Promote constructor param" leaves an orphaned docblock behind](todo/bugs.md#b5-promote-constructor-param-leaves-an-orphaned-docblock-behind)                              | Low         | Low-Medium  |
| B6  | ["Add `final`" may insert before a class brace instead of its constructor](todo/bugs.md#b6-add-final-may-insert-before-a-classs-opening-brace-instead-of-its-constructor-needs-confirmation) (needs confirmation) | Low-Medium  | Medium      |
| B7  | [Eloquent string-completion's backward paren scan has no bound](todo/bugs.md#b7-eloquent-string-completions-backward-paren-scan-has-no-bound)                              | Low         | Low         |
| B8  | [`extends` keyword completion offered inside `enum` declaration headers](todo/bugs.md#b8-extends-keyword-completion-offered-inside-enum-declaration-headers)                | Low         | Low         |
| B9  | [`class_level_signature_eq` never compares `template_param_defaults`](todo/bugs.md#b9-class_level_signature_eq-never-compares-template_param_defaults)                     | Low         | Low         |
...
| B11 | [`type_engine/subject_resolution.rs` duplicates `util::resolve_to_fqn` with a subtle difference](todo/bugs.md#b11-type_enginesubject_resolutionrs-duplicates-utilresolve_to_fqn-with-a-subtle-behavioral-difference) | Low         | Low         |
...
| B14 | [`array_pop`/`array_shift` on nested containers resolve to the container type](todo/bugs.md#b14-array_poparray_shift-on-nested-containers-resolve-to-the-container-type)     | Low-Medium  | Low         |
