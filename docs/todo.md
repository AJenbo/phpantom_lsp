...
|     | **[Bugs](todo/bugs.md)**                                                                                                                                                    |             |             |
| B238 | [`new self(...)`/`new static(...)` resolves by short name, colliding with a same-named global class](todo/bugs.md#b238-new-selfnew-static-resolves-by-short-name-colliding-with-a-same-named-global-class) | High | Medium |
| B239 | [`$this` inside a trait method resolves to the trait itself, not the using class](todo/bugs.md#b239-this-inside-a-trait-method-resolves-to-the-trait-itself-not-the-using-class) | High        | Medium-High |
| B248 | [List-destructuring against a long `array{...}` shape can bind a variable to the wrong element's type](todo/bugs.md#b248-list-destructuring-against-a-long-array-shape-can-bind-a-variable-to-the-wrong-elements-type) | High | Medium-High |
| B229 | [`instanceof` narrowing is not recognized for several common condition shapes](todo/bugs.md#b229-instanceof-narrowing-is-not-recognized-for-several-common-condition-shapes) | High        | High        |
| B246 | [`array_keys()`/`array_values()` don't propagate the source array's real key type](todo/bugs.md#b246-array_keysarray_values-dont-propagate-the-source-arrays-real-key-type) | High        | High        |
| B228 | [A condition-established narrowing of a method-call expression doesn't survive assignment, `elseif` siblings, or loop merges](todo/bugs.md#b228-a-condition-established-narrowing-of-a-method-call-expression-doesnt-survive-assignment-elseif-siblings-or-loop-merges) | High | Very High |
| B227 | [The replace function family's conditional return type never collapses](todo/bugs.md#b227-the-replace-function-familys-conditional-return-type-never-collapses) | Medium-High | Low-Medium  |
| B235 | [A lost chained-call re-narrowing misattributes the member lookup to the enclosing file's own class](todo/bugs.md#b235-a-lost-chained-call-re-narrowing-misattributes-the-member-lookup-to-the-enclosing-files-own-class) | Medium-High | Medium-High |
| B253 | [A closure parameter's type isn't inferred from a `@template`-parameterized `callable(T): X` signature](todo/bugs.md#b253-a-closure-parameters-type-isnt-inferred-from-a-template-parameterized-callablet-x-signature) | Medium-High | High |
| B230 | [`$a === $b` doesn't narrow the nullable side against a definite-type operand](todo/bugs.md#b230-a--b-doesnt-narrow-the-nullable-side-against-a-definite-type-operand) | Medium      | Medium      |
| B231 | [`array_key_exists()` doesn't narrow an optional array-shape key the way `isset()` does](todo/bugs.md#b231-array_key_exists-doesnt-narrow-an-optional-array-shape-key-the-way-isset-does) | Medium | Medium |
| B232 | [`use (&$var)` doesn't establish `$var` as defined when it's new to the enclosing scope](todo/bugs.md#b232-use-var-doesnt-establish-var-as-defined-when-its-new-to-the-enclosing-scope) | Medium | Medium |
| B233 | [`isset($var) && …` doesn't mark a possibly-undefined variable as defined for the rest of the branch](todo/bugs.md#b233-issetvar--doesnt-mark-a-possibly-undefined-variable-as-defined-for-the-rest-of-the-branch) | Medium | Medium |
| B234 | [`is_float()`/`is_int()`'s negative branch doesn't strip a member from a declared union](todo/bugs.md#b234-is_floatis_ints-negative-branch-doesnt-strip-a-member-from-a-declared-union) | Medium | Medium |
| B237 | [A vendor interface's own `@phpstan-assert-if-true Type $this` tag isn't honored](todo/bugs.md#b237-a-vendor-interfaces-own-phpstan-assert-if-true-type-this-tag-isnt-honored) | Medium | Medium |
| B240 | [`static` return type on an intersection-typed receiver drops the non-declaring interface members](todo/bugs.md#b240-static-return-type-on-an-intersection-typed-receiver-drops-the-non-declaring-interface-members) | Medium | Medium |
| B242 | [`Composer\Autoload\ClassLoader` is invisible to class resolution](todo/bugs.md#b242-composerautoloadclassloader-is-invisible-to-class-resolution) | Medium | Medium |
| B244 | [The single-argument docblock array shorthand `array<T>` defaults its key to `int`](todo/bugs.md#b244-the-single-argument-docblock-array-shorthand-arrayt-defaults-its-key-to-int) | Medium | Medium |
| B245 | [`foreach` over a bare variable skips `@phpstan-type`/`@phpstan-import-type` alias expansion](todo/bugs.md#b245-foreach-over-a-bare-variable-skips-phpstan-typephpstan-import-type-alias-expansion) | Medium | Medium |
| B252 | [`??=` doesn't route its RHS through the callable-property-invocation resolution path](todo/bugs.md#b252-doesnt-route-its-rhs-through-the-callable-property-invocation-resolution-path) | Medium | Medium |
| B236 | [`instanceof $variable` (a class-string held in a variable) narrowing is unsupported](todo/bugs.md#b236-instanceof-variable-a-class-string-held-in-a-variable-narrowing-is-unsupported) | Medium | Medium-High |
| B251 | [A closure's by-reference `use` mutation isn't tracked when the closure is invoked indirectly](todo/bugs.md#b251-a-closures-by-reference-use-mutation-isnt-tracked-when-the-closure-is-invoked-indirectly) | Medium | Medium-High |
| B247 | [No `Benevolent` union for a `foreach` key when the array's key type is genuinely unknown](todo/bugs.md#b247-no-benevolent-union-for-a-foreach-key-when-the-arrays-key-type-is-genuinely-unknown) | Low-Medium | Low |
...
| B243 | [Two more confirmed `unknown_member` false positives with an unidentified mechanism](todo/bugs.md#b243-two-more-confirmed-unknown_member-false-positives-with-an-unidentified-mechanism) | Low-Medium | Unknown |
...
| B254 | [Unused-variable check doesn't account for a closure's `use()` capture feeding a `require`d file's scope](todo/bugs.md#b254-unused-variable-check-doesnt-account-for-a-closures-use-capture-feeding-a-required-files-scope) | Low | Medium |
| B241 | [An abstract class isn't accepted where a union of exactly its known subclasses is declared](todo/bugs.md#b241-an-abstract-class-isnt-accepted-where-a-union-of-exactly-its-known-subclasses-is-declared) | Low | High |
