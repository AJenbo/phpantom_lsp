...
| B256 | [`instanceof` does not narrow an array element (`$types[$i]`)](todo/bugs.md#b256-instanceof-does-not-narrow-an-array-element-typesi) | Medium-High | Medium |
...
| B257 | [A closure parameter binds the whole container when the argument is a union of array shapes](todo/bugs.md#b257-a-closure-parameter-binds-the-whole-container-when-the-argument-is-a-union-of-array-shapes) | Medium | Medium |
...
| B246 | [`array_keys()` on an array whose keys the `+` operator merged](todo/bugs.md#b246-array_keys-on-an-array-whose-keys-the--operator-merged) | Low         | Medium      |
| B255 | [An array shape key spelled with a backslash widens the shape's key type to `int\|string`](todo/bugs.md#b255-an-array-shape-key-spelled-with-a-backslash-widens-the-shapes-key-type-to-intstring) | Low | Medium |
