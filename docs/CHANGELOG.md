...
- **Argument type mismatch diagnostics.** Flags function and method calls where an argument's resolved type is definitely incompatible with the declared parameter type. Only reports errors when there is no reasonable interpretation under which the code could work at runtime. Follows a Yes/No/Maybe framework where only "definitely wrong" produces a diagnostic. Class hierarchy is checked in both directions with full transitive interface inheritance: passing a subtype to a supertype parameter is YES (even when the relationship is through a chain of interface extends), passing a supertype where a subtype is expected is MAYBE (the value might have been narrowed), but passing a final class that doesn't match is NO (it cannot be narrowed further). Nullable arguments, bare arrays and `object`, Stringable objects as strings, objects passed to `callable` parameters (might implement `__invoke`), PHP type juggling (`int` to `string`, `numeric` to `float`), array shape supersets (including optional keys), `list`/`array`/slice interop (`X[]` is treated as `array<mixed, X>`, not `list<X>`), `class-string` covariance, backed enum values as their backing scalar type, PHPUnit `MockObject` arguments, traversable-like types as `iterable`/`Arrayable`/`array`, covariant generic type argument checking, and typed arrays to/from array shapes are all handled. Diagnostic code `type_error.argument` with ERROR severity.
...
- **Transitive interface inheritance in class hierarchy checks.** Subtype checks now walk the full interface inheritance tree. Previously, only directly-declared interfaces were checked, so a class implementing `ResponseInterface` (which extends `MessageInterface`) was not recognized as a subtype of `MessageInterface`. This affected argument type diagnostics, go-to-implementation, and any other feature that relies on the hierarchy walker.
- **Backed enums now satisfy `BackedEnum` and `UnitEnum` parameter types.** PHP automatically makes backed enums implement `BackedEnum` (and all enums implement `UnitEnum`), but the parser only recorded explicitly declared interfaces. The implicit interface is now added during parsing, so passing a backed enum where `BackedEnum` is expected no longer produces a false `type_error.argument` diagnostic. The implementation error checker skips these implicit interfaces so enums are not asked to explicitly declare `from()`, `tryFrom()`, or `cases()`.
- **Anonymous class arguments no longer produce false type errors.** Passing `new class extends Model { … }` where `Model` is expected no longer flags a type mismatch. Anonymous classes use synthetic names that are not globally indexed, so the hierarchy walker cannot verify the relationship. Since developers write anonymous classes specifically to satisfy the expected type, the diagnostic stays silent.
- **Argument type diagnostics resolve short names to FQN before comparison.** Resolved argument types from variable resolution (which may contain raw docblock short names like `SubscriptionProduct` instead of their FQN) are now normalized through the class loader before comparing against parameter types. This eliminates false positives where the types were identical but written differently.
- **Callable target cache no longer leaks across variable-based call sites.** The argument type diagnostic cached resolved callable targets by expression string (e.g. `$listener->handle`), reusing the first resolution for all occurrences in the file. When the same variable name held different types in different methods (e.g. two test methods assigning different listener classes to `$listener`), the cached parameter types from the first method were incorrectly applied to calls in later methods. Variable-based calls are now resolved fresh at each call site.
- **Parameter type enrichment no longer narrows contravariant child types.** When a concrete class declares a wider parameter type than its interface (e.g. `?int` vs `int`), the inherited type no longer overwrites the child's type. Previously, `enrich_single_parameter` copied the ancestor's type whenever the child had no docblock override, even when the ancestor's type was narrower. Now the ancestor must have a genuinely richer docblock type (matching the two-part check already used for return types). This fixes false `type_error.argument` diagnostics where `null` was passed to a parameter that the concrete class accepted as nullable but the interface declared as non-nullable.
- **Constructor PHPDoc inheritance no longer corrupts parameter types.** When a child class overrides `__construct` with a different signature, inherited parameter types are now matched by name instead of position. Previously, `CreatePaymentException::__construct(Order $order, string $message, ?Throwable $previous)` inherited `Exception`'s positional types, replacing `$message`'s `string` with `$code`'s `int`. This follows PHPStan's `PhpDocInheritanceResolver` behavior. Same-named parameters (e.g. `$previous` in both child and parent) still inherit enriched types correctly.
- **Method return types now use fully-qualified class names.** When a method returns `static`, `self`, or `$this`, the type is now expanded to the owning class's FQN instead of its short name. Previously, chaining methods on a factory (e.g. `OrderProduct::factory()->state([...])->for(...)`) produced `OrderProductFactory` without its namespace, causing the class hierarchy check to fail against `Illuminate\Database\Eloquent\Factories\Factory`.
- **Type guard narrowing applied in argument type diagnostics.** Guard clauses like `is_string()`, `instanceof`, and null checks with early return now narrow variable types at the call site. Previously, `type_error.argument` always resolved variables without considering which branch the call site was in, producing false positives when a variable was guarded before use.
...
- **Property narrowing after guard clauses.** `if (!$this->prop instanceof Foo) { return; }` now correctly narrows `$this->prop` to `Foo` in subsequent code. Previously the diagnostic cache shared a single entry for all accesses to the same `$this->prop` subject, so the first (un-narrowed) resolution was reused everywhere. Narrowing via positive instanceof, negated instanceof, and `assert()` all work for property accesses now.
- **Hover on generic methods shows concrete return type.** Hovering over a method inherited from a generic trait (e.g. `first()` from `BuildsQueries<TModel>`) now shows the substituted return type (e.g. `BlogAuthor|null`) instead of the raw template parameter name. Applies to both method return types and property type hints on generic classes.
- **False-positive undefined variable for by-reference parameters in namespaces.** Passing an undefined variable to a by-reference parameter (e.g. `initPen($pen)`) no longer produces a false "undefined variable" diagnostic when the function, static method, or constructor is in a namespace. The by-ref resolver now resolves unqualified names through the file's use-map and namespace. Instance method calls on `$this` with by-reference parameters are also handled.
- **Short-name collisions eliminated project-wide.** Every code path that constructs a `PhpType` from a class name now resolves it to a fully-qualified name at the point of construction. Previously, `new User()`, `$x instanceof Model`, `User::class` arguments, `@throws` tags, catch clauses, conditional return types, and array literal element types all produced short names that could collide across namespaces. The class hierarchy walker, throws analysis, catch-clause filtering, docblock `@throws` matching, PHPStan quickfixes, go-to-implementation, trait conflict resolution, and generic substitution all compared types by short name only. Two unrelated classes sharing a short name (e.g. `Vendor\Carbon` vs `App\Carbon`) were treated as identical. All comparisons now use fully-qualified names resolved through the class loader, use-map, and namespace.
