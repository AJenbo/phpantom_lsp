...
- **Rewritten variable resolver.** Variable type resolution now uses a single top-to-bottom pass through each function body on both the diagnostic and completion paths. The forward walker is the sole variable resolver with zero recursion and no depth limit. Analysis is significantly faster and produces fewer false-positive diagnostics.
...
- **`analyze` and `fix` commands run at consistent speed regardless of invocation style.** Running `cd project && phpantom_lsp analyze` was roughly 8× slower than `phpantom_lsp analyze --project-root project`. The resolution pipeline now checks the in-memory class cache before attempting file-based lookups, so classes parsed during Phase 1 are found immediately without re-parsing.
