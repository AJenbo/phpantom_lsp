...
## T42. Body-return inference is instance-call-only and stops at `@return mixed`

**Impact: Medium · Complexity: Medium**

```php
class Registry {
    public static function make(Widget $w)          // no declared return
    {
        return $w->clone();
    }

    /** @return mixed */
    public function fetch(Widget $w)
    {
        return $w->clone();
    }
}

$a = Registry::make($w);           // nothing at all
$b = (new Registry())->fetch($w);  // mixed
```

A method with no declared return type and no `@return` tag has its return
type read off its body by `try_infer_body_return_type`. Two things keep
that from firing where it should:

- The fallback that calls it sits at the tail of
  `resolve_owner_method_call`, so it only ever runs for an *instance*
  call. `resolve_rhs_static_call` has no equivalent, so a static call to
  the same method resolves to nothing.
- A declared `@return mixed` is returned as the answer before the
  fallback is reached. `mixed` carries no information and every type is
  a subtype of it, so reading the body can only narrow it, never
  contradict it — the same argument that already justifies inferring
  when there is no tag at all.

**Fix:** add the same fallback to the static-call path, and treat a
`mixed` return hint (native or docblock) as absent for the purpose of
reaching it, keeping `mixed` as the answer when inference produces
nothing. Both are small; the reason this is Medium rather than Low is
that widening inference to every static call and every `@return mixed`
method adds body walks on a hot path, so it needs measuring against
`projects/*` (analysis wall-clock and the `analyze` diagnostic counts)
before it can be trusted, not just a green suite.

This is the prerequisite half of
[B217](bugs.md#b217-a-mixed-returning-accessor-loses-the-type-its-arguments-decide);
the call-site-sensitive parameter seeding that bug also needs is
