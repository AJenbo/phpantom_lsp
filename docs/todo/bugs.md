...
#### B22. A union of two classes sharing a short name collapses to the first

**Impact: Medium · Effort: Medium**

Candidate classes are deduplicated by short name, not by FQN.
`ClassInfo::push_unique_arc` and `ClassInfo::push_unique`
(`src/types/mod.rs`) compare `c.name == cls.name`, and `name` holds the
unqualified class name. Two distinct classes that happen to share a
short name across namespaces are therefore treated as one, and every
member of the second is lost:

```php
namespace NsA { class Thing { public function onlyNsA(): string { return ''; } } }
namespace NsB { class Thing { public function onlyNsB(): string { return ''; } } }
...
The call reports a false positive:

```
Method 'onlyNsB' not found on class 'NsA\Thing'
```

Completion on `$maker->make()->` offers `onlyNsA()` alone, and
go-to-definition on `onlyNsB` has nothing to jump to. Same-short-name
classes are common in real projects (an `Exception`, `Client`, `Config`,
or `Response` per namespace), so any union that spans two of them
resolves to half its members.

The dedup itself is wanted: it is what stops a fluent chain through a
union return type from doubling the receiver set at every link. The key
is what is wrong, so the fix is to compare `fqn()` rather than `name`.
Both helpers are shared by completion and resolution, so a switch to FQN
comparison needs a pass over the call sites that currently rely on the
looser match (a resolved stub and its project counterpart can share a
short name and legitimately need collapsing).

**Reproduce:** the snippet above, checked with unknown-member
diagnostics and with completion after `make()->`.
